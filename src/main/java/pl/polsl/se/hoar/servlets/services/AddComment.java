        ...
        try ( PrintWriter out = response.getWriter()) {
           ...
           if( obj.isNull("task"))
           {
                ...
                out.print(new JSONObject().put("status", "error")
                        .put("message", "Task jest inwalida") );
                return;
           }
            ...
            {
                out.print(new JSONObject().put("status", "error")
                        .put("message", "User in session is a null ") );
                return;  
            }
            ...
            for (Worker tmp : user.getWorkers()) {
                System.out.println(tmp.getOrganisation());
            ...
            comment.setWorker(worker);
            comment.setCommentDate( java.sql.Date.valueOf( java.time.LocalDate.now() ) );
            comment.setCommentTime( java.sql.Time.valueOf(java.time.LocalTime.now()));
