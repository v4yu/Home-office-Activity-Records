 ...
 * @author FRANC
 */
@WebServlet(name = "SetManager", urlPatterns = {"/SetManager"})
public class SetManager extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try ( PrintWriter out = response.getWriter()) {
            ...
            if(obj.isNull("unit") ||obj.isNull("worker") )
            {
              out.print(new JSONObject().put("status", "error")
                        .put("message", "invalid unit or worker"));
                return;   
            }
            
            Unit unit = em.find( Unit.class , obj.getLong("unit") );
            Worker worker = em.find(Worker.class, obj.getLong( "worker" ));
            
            if( !unit.getWorkers().contains(worker) ){
                out.print( new JSONObject().put("status", "error").put("message", "worker is not in unit") );
                return;
            }
            
            em.getTransaction().begin();
            unit.setManager(worker);
            worker.setPermissions( worker.getPermissions() | Worker.PERMISSION_MANAGER );
