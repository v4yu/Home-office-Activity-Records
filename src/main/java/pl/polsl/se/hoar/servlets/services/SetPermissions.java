 ...
 * @author FRANC
 */
@WebServlet(name = "SetPermissions", urlPatterns = {"/SetPermissions"})
public class SetPermissions extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try ( PrintWriter out = response.getWriter()) {
            ...
            Worker worker = em.find(Worker.class, obj.optLong("worker", -1));
            if(worker == null){
             out.print( new JSONObject().put("status", "error").put("message", "invalid worker id"));
                return;
            }
            int permissions = worker.getPermissions();
            try{
                if(obj.getBoolean("admin"))
                    permissions |= Worker.PERMISSION_ADMIN;
                else
                    permissions &= ~Worker.PERMISSION_ADMIN;
            }catch( JSONException e ){}
            
            try{
                if(obj.getBoolean("dictionary"))
                    permissions |= Worker.PERMISSION_DICTIONARY;
                else
                    permissions &= ~Worker.PERMISSION_DICTIONARY;
            }catch( JSONException e ){}
            
            try{
                if(obj.getBoolean("hr"))
                    permissions |= Worker.PERMISSION_HR;
                else
                    permissions &= ~Worker.PERMISSION_HR;
            }catch( JSONException e ){}
