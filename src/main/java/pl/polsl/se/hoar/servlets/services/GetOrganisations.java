 ...
 * @author FRANC
 */
@WebServlet(name = "GetOrganisations", urlPatterns = {"/GetOrganisations"})
public class GetOrganisations extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try ( PrintWriter out = response.getWriter()) {
           ...
           if(request.getSession().getAttribute("user") == null)
            ...
            user = em.merge( user );
            em.refresh(user);
            out.print( new JSONObject().put("organisations", 
                    user.getWorkers().stream().map( (Worker worker)->{
                        return new JSONObject().put( "id", worker.getOrganisation().getId() ).put("name", worker.getOrganisation().getName());
            } ).toList()  ) );
        }
    }
