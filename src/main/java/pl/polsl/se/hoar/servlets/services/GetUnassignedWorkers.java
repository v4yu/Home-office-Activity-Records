 ...
 * @author FRANC
 */
@WebServlet(name = "GetUnassignedWorkers", urlPatterns = {"/GetUnassignedWorkers"})
public class GetUnassignedWorkers extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try ( PrintWriter out = response.getWriter()) {
            ...
            out.print( new JSONObject().put("status", "ok").put( "workers", 
                    worker.getOrganisation().getWorkers().stream()
                            .map( (Worker w)->{em.refresh(w); return w; } )
                            .filter( (Worker w) ->{
                        return w.getUnit()==null;
                    })
                    
                    .map( Worker::getShortcut ).toList()
                    ) );
        }
    }
